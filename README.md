...
## ⚠ Важно: генерация GIF-анимации

Создание GIF-анимации может занимать значительное время в зависимости от выбранных параметров.

На скорость генерации влияют:

- `t_max` — общее время моделирования  
- `dt` — шаг интегрирования (чем меньше шаг, тем больше кадров)  
- `fps` — количество кадров в секунду в итоговой анимации (не значительно)
- `tail` — длина отображаемого хвоста траектории (не значительно)


#### Главную роль играет количество кадров. Чем их больше, тем дольше будет создание GIF-анимации.

Количество вычисляемых кадров примерно равно:



```bash
число_кадров ≈ t_max / dt
```
...
```bash
python main.py --vy0 0.8 --t_max 20 --dt 0.002 --save_gif --fps 60 --tail 200
````

генерирует около 10 000 кадров и будет выполняться около 8 минут (время будет зависеть от мощности пк).

---

### Рекомендации

Если GIF создаётся слишком долго:

* увеличьте `dt` (например, до `0.005` или `0.01`)
* уменьшите `t_max`

Пример более быстрого варианта:

```bash
python main.py --vy0 0.8 --t_max 4 --dt 0.01 --save_gif --fps 40 --tail 100
```

такой запрос выполнится примерно за 20 секунд (обработка 400 кадров).
